...
public class MysqlHelper {
	 ...
	 * @throws DBException
	 */
	public static <T> T query(Class<T> beanClass, String sql, Object... params) throws SQLException {
		printIn(sql);
		return (T) queryRunner.query(getConnection(), sql, isPrimitive(beanClass) ? scalarHandler : new BeanHandler<T>(beanClass), params);
	}
	...
	public static <T> List<T> queryList(Class<T> beanClass, String sql, Object... params) throws SQLException {
		printIn(sql);
		return (List<T>) queryRunner.query(getConnection(), sql, isPrimitive(beanClass) ? columnListHandler : new BeanListHandler<T>(beanClass), params);
	...
	public static int excute(String sql, Object... params) throws SQLException {
		...
		if (sql.toLowerCase().indexOf("insert") == -1) {
			...
			queryRunner.update(getConnection(), sql, params);
			Number num = (Number) queryRunner.query(getConnection(), "select last_insert_id()", scalarHandler);
